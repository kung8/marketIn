module.exports = {
    getProfile: async (req,res) => {
        ...
        console.log(id)
        const edProfile = await db.profiles.get_education({user_id:id});
        const workProfile = await db.profiles.get_work({user_id:id});
        const skillsProfile = await db.profiles.get_skills({user_id:id});
        const langProfile = await db.profiles.get_languages({user_id:id});
        const projProfile = await db.profiles.get_projects({user_id:id});
    ...
    createProfile: async (req,res)=>{
       ...
       let projProfile = [];
        ...
        for(let i=0;i<education.length;i++){
            ...
            let res = await db.profiles.create_education({sch_name,major,ed_level,sch_loc,grad_date,sch_logo,user_id:id})
            edProfile.push(res);
        };
        ...
        for(let i=0;i<work.length;i++){
            ...
            let res = await db.profiles.create_work({emp_Name,position,emp_Loc,hire_Date,end_Date,emp_Logo,user_id:id});
        ...
        for(let i=0;i<skills.length;i++){
            ...
            let res = await db.profiles.create_skills({skill,user_id:id});
        ...
        for(let i=0;i<languages.length;i++){
            ...
            let res = await db.profiles.create_languages({language,user_id:id});
        ...
        for(let i=0;i<projects.length;i++){
            ...
            let res = await db.profiles.create_projects({project,user_id:id});
